...
def create_drawing():
    ...
    with open(os.path.dirname(os.path.dirname(os.path.abspath(__file__))) + '/user/users.json', 'r') as users_file:
        ...
        for user_data in users:
            if user_data['username'].lower() == requester.lower():
                ...
                user_data['drawing_number'] = int(user_data['drawing_number']) + 1
    ...
    with open(os.path.dirname(__file__) + '/drawings/' + artist + '/' + drawing_number + '.png', 'wb') as drawing_file:
    ...
    # Create folder for artist's drawing information if one does not already exist
...
def update_drawing_info(artist, drawing_id):
    data = request.get_json()
    # If request is for viewing the drawing, increase view count without requiring user to be logged in
...
def read_all_user_drawings(artist):
    ...
    # Sort all drawings by newest to oldest creation time
    all_drawings.sort(key = os.path.getctime, reverse = True)
    # Return requested drawings' file paths as '[artist]/[drawing_id].png', replacing artist's member_id with username
